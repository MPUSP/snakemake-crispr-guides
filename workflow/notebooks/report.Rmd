...
df_guides %>%
  ...
  labs(
    ...
    subtitle = paste0("Colored arrows: guide RNAs, gray line: TSS window, black arrow: transcript.\n",
...
## Guides with multiple targets
 ...
 - the following figure gives an overview about the number of multi-target guides, if they were not filtered
...
if ("multi_target" %in% colnames(df_guides)) {
  ...
  print("No guides with multiple targets found in data set.")
}
```

## Targets where no guides were designed

- in some cases the design of guides is not possible
- for example if the target (gene) is very short, e.g. below 100 bp, and no PAM site can be found
- or if genes are duplicated, leading to the removal of multi-target guiges (see previous section)
- this table and figure lists the targets where no guides were found, if any

```{r, echo = FALSE}
df_guides_fail <- read_csv(snakemake@input$failed, show_col_types = FALSE)
if (nrow(df_guides_fail) > 0) {
  df_guides_fail
  df_guides_fail %>%
    ...
    labs(
      x = "position [kbp]", y = "",
      title = paste0("Location of targets where no guides could be found (", nrow(df_guides_fail), ")"),
