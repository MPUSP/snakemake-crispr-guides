...
df_guides %>%
  ...
  geom_smooth(color = grey(0.3), fill = custom_colors[1]) +
  labs(x = "", y = "",
...
```{r, echo = FALSE, warning = FALSE, fig.width = figwidth, fig.height = figwidth}
df_guides %>%
  filter(tx_name %in% unique(df_guides$tx_name)[1:10]) %>%
  mutate(start = abs(start - tss_pos), end = abs(end -tss_pos)) %>%
  mutate(tx_name = factor(tx_name, unique(tx_name))) %>%
  ggplot(aes(x = start, y = 0, xend = end, yend = 0)) +
  geom_segment() +
  facet_wrap(~ tx_name, ncol = 1) +
  coord_cartesian(ylim = c(-0.5, 0.5)) +
  labs(x = "", y = "",
    title = "Guide RNA location in TSS window, selected genes",
    subtitle = "Arrow shows direction of the guide") +
  custom_theme() +
